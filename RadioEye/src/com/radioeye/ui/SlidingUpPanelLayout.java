...
public class SlidingUpPanelLayout extends ViewGroup {
	 ...
	 */
	private static final int DEFAULT_PANEL_HEIGHT = 0; // 0dp;
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (childCount > 2) {
			...
			com.radioeye.utils.Log
					.e("onMeasure: More than two child views are not supported.");
	...
	private boolean expandPane(View pane, int initialVelocity,
			float mSlideOffset, Panelcallback callback) {
		
		System.err.println(mFirstLayout+"===");
		
 	return 
 				  smoothSlideTo(mSlideOffset, initialVelocity, callback) ||  mFirstLayout;
	...
	private boolean collapsePane(View pane, int initialVelocity,
		...
		return mFirstLayout || smoothSlideTo(1.f, initialVelocity, callback);
	}
	 ...
	 */
	public boolean collapsePane(Panelcallback callback) {
		return collapsePane(mSlideableView, 0, callback);
	}
	 ...
	 *         of expading
	 */
	public boolean expandPane(Panelcallback callback) {
		return expandPane(0, callback);
	}
	 ...
	 *         of expading
	 */
	public boolean expandPane(float mSlideOffset, Panelcallback callback) {
		System.err.println("==expandPane==1=====");
		if (!isPaneVisible()) {
			System.err.println("==expandPane==2=====");
			showPane();
		}
		return expandPane(mSlideableView, 0, mSlideOffset, callback);
	}
	...
	public boolean isPaneVisible() {
		System.err.println("==isPaneVisible==1=====");
		if (getChildCount() < 2) {
			System.err.println("==isPaneVisible==2=====");
			return false;
		}
		System.err.println("==isPaneVisible==3=====");
		View slidingPane = getChildAt(1);
		System.err.println("==isPaneVisible==4=====");
		
		slidingPane.getVisibility();
		
		return slidingPane.getVisibility() == View.VISIBLE;
	}

	public void showPane() {
		System.err.println("==showPane==1=====");
		if (getChildCount() < 2) {
			return;
		}
		View slidingPane = getChildAt(1);
	...
	boolean smoothSlideTo(float slideOffset, int velocity,
			Panelcallback callback) {
		System.err.println("==smoothSlideTo==1=====");
		if (!mCanSlide) {
			// Nothing to do.
			System.err.println("==smoothSlideTo==2=====");
			return false;
		}
